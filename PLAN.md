...
- [x] A1: Compute column statistics in core audit
    - Depends on: none
...
- [x] A2: Render profile stats in HTML report
    - Depends on: A1
...
- [x] A3: Include profile stats in Excel and PDF reports
    - Depends on: A1
...
- [x] A4: Expose profiling in typed API
    - Depends on: A1
...
- [x] B1: Add directory/glob input resolution
    - Depends on: none
...
- [x] B2: Run audit across multiple files and merge results
    - Depends on: B1
...
- [x] B4: Add `--recursive` flag and document
    - Depends on: B3
...
- [x] C1: Add structured exit codes
    - Depends on: none
...
- [x] C2: Create GitHub Action definition
    - Depends on: C1
...
- [x] C3: Add SARIF output for GitHub Code Scanning
    - Depends on: C1
...
- [x] C4: Document CI usage in README
    - Depends on: C2, C3
...
- [x] All sub-tasks checked off
- [x] `pytest` passes with new profiling and multi-file tests
- [x] `ruff check .` passes
- [x] `data-hygiene-audit --input samples/input/ --output ./reports` audits all files in directory
- [x] HTML report shows column stats (cardinality, uniqueness)
- [x] `--fail-under 70` exits non-zero on low-scoring data
- [x] GitHub Action YAML is valid and documented

---

## Improvement History

### 2026-05-22 — Improvement pass (audit + plan)
- **Trigger:** User-initiated `/improve` pass
- **What was reviewed:** All modules, workflow files, tests, dependencies, git hygiene, security (3 agents), code quality (3 agents)
- **Security findings:** 0 real vulnerabilities (2 candidates filtered at 2/10 confidence)
- **Code quality findings:** 16 actionable items (4 critical, 7 important, 5 nice-to-have)
- **What was fixed:** Import ordering (ruff I001) in html.py and pdf.py
- **Deferred:** All 16 findings queued for next session (see HANDOFF.md for full list)
